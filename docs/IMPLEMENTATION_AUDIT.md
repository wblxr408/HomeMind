...
相较于上一轮审计，以下事项已经有明确推进，不应再按“完全未实现”处理：

- 已同步完成：mock 数据全链路可跑通、设备控制与场景切换前端入口、最小 TAP 规则页、SVG 户型图上传与元数据存储、设备映射导入与基础编辑、SSE 辅助链路、Schema 基础校验、启动脚本同步、文档迁入 `docs/`。
- 已部分完成：新版 Web UI 原型适配、空间配置页直出、户型图预览链路、上下文压缩与固定 JSON 输出、自动化规则执行闭环。
- 仍未完成：端云协同正式主链路、结构化记忆与偏好持久化闭环、规则冲突检测、命中统计、性能与资源 benchmark、方言归一化。

审计口径说明：

- “已同步完成”表示仓库中已有代码或脚本支撑，且当前可在功能层看到入口。
- “已部分完成”表示代码已进入仓库，但仍存在 bug、联调缺口或验证不足。
- “仍未完成”表示当前仓库还不能严谨宣称已交付。
