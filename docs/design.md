...
本设计文档最初包含较多“目标态”描述。结合当前仓库，现补充真实落地状态如下：

- 已落地：家庭场景主链路、BSR/LSR/LLM 基础链路、Web 控制台、mock 数据运行、设备控制与场景切换、最小 TAP 规则页、SVG 户型图上传、设备映射基础编辑、Schema 基础校验。
- 部分落地：新版 UI 原型适配、空间配置与户型图可视化、上下文压缩、SSE 长任务输出、规则自动化执行闭环、记忆与偏好结构化写回骨架。
- 未落地：文中表述的完整端侧模型压缩与量化证明、稳定本地 LLM 部署闭环、结构化长期记忆闭环、规则冲突治理、性能 benchmark、跨家庭之外场景扩展。

因此，阅读本设计文档时应区分“当前已实现能力”和“目标架构能力”，不能把全文都视为已经完成的实现说明。
