...
class Alert:
    def __init__(self, alert_id: str, level: str, message: str, metric: str = "", value: float = 0.0, threshold: float = 0.0):
    ...
    def to_dict(self) -> dict:
        return {
            "alert_id": self.alert_id, "level": self.level, "message": self.message,
...
class Alerter:
    def __init__(self, metrics):
        ...
        self._log_dir = Path(STORAGE_CONFIG.get("logs_dir", "logs"))
        self._alert_log_path = self._log_dir / "alerts.log"
        self._ws_handlers: List[Callable] = []
        self._log_dir.mkdir(parents=True, exist_ok=True)

    def register_ws_handler(self, handler: Callable):
    ...
    def check(self) -> List[Alert]:
        ...
        for key, message in metric_alerts.items():
            if key not in self._active_alerts:
                ...
                alert = Alert(alert_id=key, level=level, message=message, metric=key)
