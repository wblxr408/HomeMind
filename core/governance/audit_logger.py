...
class AuditLogger:
    ...
    def _protect_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._encrypt_fields:
            for fld in SENSITIVE_AUDIT_FIELDS:
                protected[fld] = self._encrypt_value(protected.get(fld, ""))
            encrypted = 1
        ...
        try:
            with _audit_lock:
                ...
                for row in cursor.fetchall():
                    ...
                    result = dict(stored)
                    encrypted_flag = bool(result.get("encrypted", 0))
                    if encrypted_flag:
                        for fld in SENSITIVE_AUDIT_FIELDS:
                            if fld in result:
                                result[fld] = self._decrypt_value(result[fld])
