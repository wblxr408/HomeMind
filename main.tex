...
\begin{enumerate}
    ...
    \item \textbf{可量化的系统测试结果。} 当前测试结果表明，系统在端侧模拟环境下具备毫秒级 API 响应能力，批量指令数据集总体匹配率为 100.00\%，端侧 API P95 延迟为 10.643 ms；同时，项目通过 110 个自动化回归测试用例，为后续测试结果分析提供了量化基础。
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\bfseries}p{4cm}X>{\centering\arraybackslash}p{2.5cm}}
        ...
        指标 & 说明 & 结果 \\
        \midrule
        自动化测试通过数量 & pytest 全量回归测试覆盖单元、模块、Web API、数据韧性、户型图、设备管理与场景管理流程 & 110 passed \\
...
当前自动化测试共收集并通过 110 个用例，覆盖核心算法模块、Web API、Mock 主流程、TAP 自动化、语音反馈、本地敏感数据保护、知识库恢复、SVG 户型图管理、设备映射、设备注册表增删改查、场景配置增删改查以及空间设备强校验等范围。该结果表明项目已经具备可重复执行的回归测试基础。进一步的 87 条批量指令数据集评测显示，系统总体匹配率由修复前的 85.06\% 提升至 100.00\%，自动化任务创建、聊天回复、澄清请求、英文/口语归一化、场景切换、设备控制、不支持设备识别和上下文跟随八类样本均达到 100\% 通过率。
...
场景四（户型图空间配置）：用户上传 SVG 户型图，导入设备映射数据，系统按房间区域分组标注设备图标；用户可通过 \texttt{areaName}/\texttt{areaNames} 为每个区域设置自己的名称，例如把 \texttt{living\_room} 命名为“影音区”，之后“打开影音区灯”会按该自定义区域名解析。
