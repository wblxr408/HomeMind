...
\begin{titlepage}
    ...
    \vfill
    \includegraphics[width=0.6\linewidth]{homemind_images/Logo图.png}
    \captionof{figure}{HomeMind Logo}
...
=======
\indent 随着端侧 AI 算力持续提升，智能家居开始从“设备联网”进入“家庭终端智能体”阶段。HomeMind 面向 4 GB 端侧内存预算，构建一套能够长期运行在家庭终端上的端侧 Agent 主链路，将语音交互、自然语言理解、上下文记忆、偏好适配、结构化决策与设备执行组织为统一闭环。本套系统 mock 后端下 Web Agent 完成初始化和查询后的 RSS 峰值为 982.98 MB，Core Agent 峰值为 1211.88 MB；启用 Qwen2.5-1.5B-Instruct-Q4\_K\_M 的 llama.cpp 本地推理链路后，Full Local 峰值为 2209.48 MB，满足要求。
>>>>>>> Stashed changes

在总体架构上，HomeMind 采用“端侧优先、端云协同增强”的设计思路：常规设备控制、场景切换和信息查询先由本地 BSR 召回与 LSR 重排序缩小候选空间，再由 Router 按置信度决定本地直达、云端协同或澄清询问；低代码场景新建、TAP 自动化规则生成和多轮 Change 纠正等开放式生成任务则允许云端前置生成结构化草案，再回到端侧完成设备、空间、协议、Schema 与安全校验。对于门锁、门禁、摄像头布防/撤防等家庭安全相关指令，系统不采用“高置信度直接执行”策略，而是无论置信度高低都先进入澄清确认链路。在理解链路上，BSR 广召回模块以规则、向量、历史记忆三路并行方式快速定位候选动作；LSR 轻量重排序模块以 5 维环境与偏好特征完成上下文感知排序；DQN 主动推荐模块则负责在合适时机给出场景级建议。在记忆设计上，系统采用“会话上下文 + 结构化偏好 + 可检索长期记忆”的三层记忆结构，将家庭场景中天然短小、事件驱动的经验条目作为原子化记忆单元直接存储与检索，避免长文档切块管理开销，同时显著降低写回复杂度与索引成本。
...
=======
在总体架构上，HomeMind 采用“端侧优先、端云协同增强”的设计思路：常规设备控制、场景切换和信息查询先由本地规则、BSR 召回、LSR 重排序与本地 GGUF 结构化决策尝试闭环；只有当本地候选不足、Router 准备进入澄清，或本地结构化决策置信度不足时，才触发最小必要上下文上云补救。低代码场景新建、TAP 自动化规则生成和多轮 Change 纠正等开放式生成任务则允许云端前置生成结构化草案，再回到端侧完成设备、空间、Schema 与安全校验。当前主链已在 BSR-LSR-Router-LLM-Execution 扩展补齐输入注入检测、动态工具注册、运行时策略引擎、零信任身份上下文、审计记录与事务回滚能力。对于门锁、门禁、摄像头布防/撤防等家庭安全相关指令，无论置信度高低系统都先进入澄清确认链路。在理解链路上，BSR 广召回模块以规则、向量、历史记忆三路并行方式快速定位候选动作；LSR 轻量重排序模块以 5 维环境与偏好特征完成上下文感知排序；DQN 主动推荐模块则负责在合适时机给出场景级建议。在记忆设计上，系统采用“会话上下文 + 结构化偏好 + 可检索长期记忆”的三层记忆结构，并将长期记忆升级为按来源分库、带可信度分级、支持冲突检测、时序摘要和语义压缩的本地 RAG 组件，以短条目、事件化、可写回的方式沉淀家庭经验。
...
\vspace{1em}
\noindent\textbf{关键词：} 端侧轻量化Agent；本地知识库；RAG；端云协同；Schema 校验；深度Q网络
...
Architecturally, HomeMind follows an “edge-first with cloud enhancement” strategy. For ordinary control tasks, the system first uses local BSR recall and LSR re-ranking, then lets the Router decide whether to complete locally, call the cloud, or ask for clarification. For open-ended low-code tasks such as scene creation, TAP rule generation, and multi-turn Change correction, the cloud can be invoked earlier to draft structured configurations, which are then validated locally against devices, spaces, protocols, schemas, and safety constraints. Safety-sensitive actions such as door locks, access control, and security arming or disarming always require clarification regardless of confidence. In memory design, the system adopts a three-layer structure of session context, structured preferences, and retrievable long-term memory. Household experience is stored as short atomic memory items rather than long-document chunks, which keeps write-back simple, retrieval efficient, and indexing overhead low for edge deployment.
...
<<<<<<< Updated upstream
在该设计思路下，系统需要同时满足以下技术约束：第一，满足赛题 4 GB 内存上限以及系统常态业务预算 1--2 GB 的资源限制，内存验证采用进程 RSS 峰值作为赛题判定口径；在 Python 3.10 + PyTorch 2.6.0 测试环境下，当前端侧 Web Agent 加载 embedding 模型并完成查询后的实测峰值为 1240.40 MB，Core Agent 实测峰值为 1223.14 MB，均低于 4 GB 赛题上限；第二，在家庭交互场景下维持较低推理时延，当前测试结果表明端侧推理 P50 延迟低于 50 ms、P99 延迟低于 200 ms；第三，兼顾本地直达能力与复杂场景处理能力，本地直达成功率达到 92\% 以上；第四，通过 Router 置信度路由、Token 压缩和 Schema 校验控制云端协同范围，使云端调用成本降低 60\% 以上，并减少隐私暴露；第五，建立安全敏感设备的强制澄清规则，门锁、门禁、安防布撤防等动作必须先获得用户确认，不能仅凭模型置信度或召回得分直接执行。
=======
家庭场景具有高频交互、强上下文依赖、隐私敏感、成员表达差异大和结果可即时验证等特征，对端侧智能体提出了更高要求。用户希望通过自然语言直接完成设备控制、场景切换、信息查询和自动化配置，希望交互接近本地即时响应，希望家庭偏好、历史习惯和上下文状态沉淀在本地，希望系统能够理解口语、方言与跨年龄表达。围绕这些真实需求，HomeMind 采用“端侧优先、端云协同增强”的总体路线：高频明确任务优先由本地规则、候选排序和本地 GGUF 决策闭环处理；只有当本地候选不足、结构化决策不稳定或即将进入澄清链路时，才触发云端兜底补救；复杂生成任务则允许云端先输出结构化草案，再由端侧完成校验、执行与记忆更新。
>>>>>>> Stashed changes
...
=======
从资源效率看，HomeMind 以轻量部署和持续运行成本控制为前提进行设计。系统在 Python 3.10 + PyTorch 2.6.0 测试环境下，mock 后端 Web Agent 完成初始化和查询后的实测峰值为 982.98 MB，Core Agent 峰值为 1211.88 MB；在 \texttt{used\_pytorch} 环境中加载 Qwen2.5-1.5B-Instruct-Q4\_K\_M 的 llama.cpp 本地链路后，Full Local 峰值为 2209.48 MB，整体仍低于 4 GB 端侧内存上限。批量数据集评测中 87 条指令均匹配通过，总体 API 平均延迟为 6.211 ms、P95 延迟为 10.643 ms。基于这一组结果，HomeMind 已具备端侧主链低延迟、可回归和可部署的工程基础。
>>>>>>> Stashed changes

本项目的价值体现在一条完整、可解释、可验证的端侧智能体闭环：输入经由本地感知与语言归一化后，常规控制任务先由 BSR 广召回和 LSR 轻量精排完成候选理解，再由 Router 决定本地直达、云端协同或澄清询问路径；低代码配置类任务则由云端先生成草案，再由端侧完成设备、空间、协议和安全校验，最终通过结构化决策与工具执行落到真实设备控制、空间映射和记忆更新。这一设计直接对应赛题要求的场景理解与任务规划、工具调用与资源整合、多轮对话与状态保持、高效推理与实时响应四大能力，也使系统能够在家庭成员表达差异较大的条件下保持较低使用门槛。
...
\begin{figure}[H]
    ...
    \begin{placeholderbox}[图占位符]
        \centering
        \vspace{2.2cm}
        {\Large \textbf{HomeMind 端侧智能体总体方案概览}}\\[0.6em]
        {\small 原图文件 \texttt{1-2.png} 在部分编译环境中无法被识别，可替换为有效 PNG/PDF 后改回 \texttt{\string\includegraphics}。}
        \vspace{2.2cm}
    \end{placeholderbox}
...
<<<<<<< Updated upstream
        ...
        轻量部署能力 & 在端侧资源约束下维持低延迟推理与多模块协同 & Python 3.10 + PyTorch 2.6.0 测试环境实测 RSS 峰值 1240.40 MB，P50 $<$ 50 ms，P99 $<$ 200 ms \\
        系统闭环能力 & 形成从感知、决策、执行到反馈写回的完整链路 & 统一 execute 接口、WebSocket 实时反馈、RAG 写回与偏好学习闭环 \\
        隐私安全能力 & 在最小必要上传前提下实现复杂意图协同，并保证结果可控；安全敏感设备必须先澄清确认 & Token 节省约 60\%，云端结果必须经过 Schema 校验后方可执行，门锁/门禁/安防类动作无论置信度均不直接执行 \\
=======
        创新能力 & 构建区别于单点设备控制的端侧智能体主链，将检索、路由、工具、记忆与安全治理整合为统一系统 & 动态工具注册、来源分库 RAG、运行时策略引擎、审计检索与事务回滚全部接入主链 \\
        场景价值能力 & 将自然语言输入稳定映射为候选动作、结构化决策与可执行指令，形成家庭智能入口 & 批量数据集中 60/87 条请求进入 local 路由，覆盖设备控制、场景切换、信息查询三类高频意图 \\
        语言适配能力 & 对家庭场景中的口语表达、常见方言词和英文控制表达进行识别与归一化，提升全家庭可用性 & 通过语言归一化层将“闷得慌”“热死咯”“困告了”等表达映射为标准命令词 \\
        轻量成本能力 & 在端侧资源约束下维持低延迟推理与多模块协同，并压缩云端调用开销 & Python 3.10 + PyTorch 2.6.0 测试环境下 mock Web Agent 峰值 982.98 MB，Full Local + llama.cpp 峰值 2209.48 MB；总体 API 平均延迟 6.211 ms，P95 为 10.643 ms \\
...
围绕上述目标，系统在能力设计上进一步强调六项要求：高频明确指令应优先在端侧闭环完成；低代码场景新建、自动化规则生成和 Change 纠正等开放式生成任务可以先调用云端生成草案，再在端侧进行校验和确认；语言输入需要经过口语和方言归一化处理，以降低不同家庭成员的使用门槛；工具调用、设备执行与反馈写回应被纳入同一主链；会话上下文与长期偏好需要联合建模；系统输出必须结构化且可校验，且家庭安全相关动作必须进入澄清确认，以保证后续执行阶段的安全性与稳定性。
...
<<<<<<< Updated upstream
    ...
    \item \textbf{置信度驱动的端云协同路径。} 系统依据任务类型与置信度将请求划分为本地直达、云端协同和澄清询问三类处理分支：常规控制任务先经 BSR/LSR 后再决定是否上云，避免把高频明确指令交给高成本模型；低代码场景新建、自动化规则生成和 Change 纠正等生成型任务则允许云端前置生成草案，再由端侧校验。该策略在响应时延、调用成本与处理鲁棒性之间取得平衡。
=======
...
\item \textbf{本地优先、失败再上云的协同路径。} 系统依据任务类型、候选充分性与结构化决策置信度将请求划分为本地闭环、云端协同和澄清询问三类处理分支：常规控制任务先由本地规则、BSR/LSR 与本地 GGUF 尝试完成结构化决策，只有在本地候选不足、Router 准备进入澄清，或本地结构化决策置信度不足时才触发最小必要上下文上云补救；低代码场景新建、自动化规则生成和 Change 纠正等生成型任务则允许云端前置生成草案，再由端侧校验。端侧执行前还需经过身份授权、策略评估、异常频率检测与审计登记，从而在响应时延、调用成本与处理鲁棒性之间取得平衡。
>>>>>>> Stashed changes
    ...
    \item \textbf{结构化可控的执行链路。} 系统将最小必要上传、本地加密存储、固定 JSON 决策输出与命令约束校验嵌入主链，使模型输出天然面向执行层消费，并稳定落地为设备动作、场景编排、TAP 规则和空间设备映射。对于门锁、门禁、摄像头布防/撤防等家庭安全动作，执行链路额外设置高优先级澄清门控，必须获得用户明确确认后才允许进入工具调用。
    \item \textbf{体现人本导向的长期适配能力。} 系统通过偏好记忆、历史纠正写回和澄清询问机制，在不增加使用负担的前提下持续贴近家庭成员的表达习惯与操作偏好，使“能用”进一步转化为“更易用、更愿意用”。
    \item \textbf{可量化的系统测试结果。} 当前测试结果表明，系统在端侧模拟环境下具备毫秒级 API 响应能力，批量指令数据集总体匹配率为 100.00\%，端侧 API P95 延迟为 10.643 ms；同时，项目通过 125 个自动化回归测试用例，为后续测试结果分析提供了量化基础。
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\bfseries}p{3.2cm}X X}
        ...
        \midrule
        高频任务需要本地完成 & 不依赖单一云端大模型即可完成主要家庭任务闭环 & 高频明确请求优先由本地规则、候选排序与本地 GGUF 闭环；本地候选不足或低置信度时再上云 \\
        ...
        终端资源受限 & 模型、检索、语音、记忆模块均需满足端侧资源约束 & 常态业务预算 1--2 GB；mock Web Agent 峰值 982.98 MB，Full Local + llama.cpp 峰值 2209.48 MB，整体低于 4 GB 上限 \\
        交互实时性要求高 & 在正常与异常场景下均保持可用且具有低延迟表现 & P50 $<$ 50 ms，P99 $<$ 200 ms，网络异常时可本地降级 \\
        隐私与安全要求严格 & 最小必要上传，且协同结果在端侧仍然受控；门锁、门禁、安防布撤防等安全动作必须澄清确认 & Token 压缩、Schema 校验、Fernet + PBKDF2-HMAC-SHA256 本地加密存储，安全敏感动作高优先级拦截 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
<<<<<<< Updated upstream
\textbf{1. 资源约束。} 在赛题 4 GB 内存上限下，系统常态业务预算控制在 1--2 GB 以内。当前 Python 3.10 + PyTorch 2.6.0 测试环境按照进程 RSS 峰值实测，Web Agent 加载 embedding 模型并完成查询后的峰值为 1240.40 MB，Core Agent 峰值为 1223.14 MB，仍处于 4 GB 上限与 1--2 GB 常态业务预算以内。为此，LSR 参数量控制在 5 MB 以下，DQN 参数量约 2,000，向量模型和语音模型均采用按需加载策略；llama.cpp 本地大模型链路需在安装 \texttt{llama\_cpp} 并提供 GGUF 模型文件后单独复测。
=======
\textbf{1. 资源约束。} 在 4 GB 端侧内存上限下，系统常态业务预算控制在 1--2 GB 以内。当前 Python 3.10 + PyTorch 2.6.0 测试环境按照进程 RSS 峰值实测，mock 后端 Web Agent 完成初始化和查询后的峰值为 982.98 MB，Core Agent 峰值为 1211.88 MB；embedding profile 峰值为 1121.27 MB，ChromaDB profile 峰值为 114.85 MB，PyTorch + DQN profile 峰值为 486.33 MB。进一步在 \texttt{used\_pytorch} 环境中加载 Qwen2.5-1.5B-Instruct-Q4\_K\_M 的 llama.cpp 本地链路后，Full Local 峰值为 2209.48 MB，仍低于 4 GB 上限。为此，LSR 参数量控制在 5 MB 以下，DQN 主推荐网络参数量为 5257，向量模型和语音模型均采用按需加载策略。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\bfseries}p{2.2cm}X X >{\centering\arraybackslash}p{1.4cm}}
        ...
        端云协同 & 常规控制先由本地规则、候选排序与本地 GGUF 尝试闭环，失败或低置信度时再上云；低代码生成类任务可云端前置生成草案 & Router、Token 压缩、云端协同模块、本地校验模块 & 中高 \\
...
从模块分工上看，HomeMind 的主链是一条职责清晰、前后咬合的端侧 Agent 链路：语言与语音识别负责把自然输入转成可计算文本，BSR 负责高召回地找出候选动作，LSR 负责把环境状态和用户偏好注入排序，Router 负责决定继续本地闭环、触发云端兜底还是进入澄清询问，LLM 负责输出结构化决策，TAP 引擎和设备执行层负责把决策落为真实动作，DQN 负责补足主动推荐能力，而上下文记忆与本地 RAG 则贯穿整条链路，持续提供历史证据和偏好增益。这种架构使每个模块都承担明确任务，让端侧资源能够被精确使用。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\bfseries}p{2.7cm}X X X}
        ...
        技术策略 & 适用位置 & 采用方式 & 项目收益 \\
        \midrule
        云端大模型能力 & 两类场景：常规控制中的受控兜底补救；低代码场景/规则/Change 纠正的草案生成 & 常规控制先由本地规则、BSR/LSR 与本地 GGUF 尝试闭环，只有在本地候选不足或结构化决策不稳定时才由 Router 触发上云；低代码生成任务可先上云生成结构化草案，再回到端侧做设备、空间、协议、Schema 与安全校验 & 增强复杂意图理解和低代码生成能力，同时控制时延、成本与隐私暴露范围 \\
...
\node[cloudbox, right=0.2cm of router.east, anchor=west, yshift=-0.2cm] (cloud) {%
    ...
    {\sffamily\fontsize{8.6pt}{10.4pt}\selectfont\bfseries\color{hmdark} 模糊裁决 / 草案生成}\\[-1pt]
    {\sffamily\fontsize{7.6pt}{9pt}\selectfont\color{hmgray} 固定 JSON + 本地校验}
};
...
\draw[assistarrow] (router.east) -- node[above, labelfont, pos=0.48] {控制类：重排后上云} (cloud.west);
\draw[assistarrow] (cloud.south west) to[out=225, in=0] node[below, labelfont, pos=0.42] {草案/裁决回注校验} (llm.east);
...
基于这一组合策略，HomeMind 采用七层端侧智能体架构，自上而下依次为交互层、BSR 理解层、LSR 理解层、Router 决策层、LLM 决策层、执行层和记忆层，并在侧边设置云端协同模块承接模糊意图补救与低代码草案生成。系统主链总体上受控串行，BSR 内部采用规则、向量、历史三路并行召回，记忆层向理解层和决策层回注检索结果，因此整体架构兼具“主链可控”和“局部并行增强”两种特征。云端协同的位置由任务类型决定：面向设备控制和场景切换的高频明确指令，系统先由本地规则、BSR/LSR 与本地 GGUF 尝试完成结构化决策，只有在本地候选不足、Router 准备进入澄清或本地结构化决策置信度不足时，才触发云端补救；面向场景新建、TAP 规则生成和多轮 Change 纠正的开放式配置任务，云端位于结构化草案生成阶段之前，但草案必须回到端侧校验后才可保存或执行。方言和口语理解前置到交互层的语言归一化环节，与后续召回、精排和偏好记忆共同构成面向家庭成员差异的人本交互主链。
...
从系统设计角度看，这一架构有四个关键价值。第一，交互层与执行层分别处理“输入来源多样”和“输出动作落地”问题，形成理解到行动的闭环；第二，语言归一化层前置处理口语和方言表达，使不同家庭成员可以沿用自然表达方式；第三，BSR + LSR 的双层理解链路承担大部分高频任务，显著降低大模型调用压力；第四，Router 将“是否上云”从固定策略变成任务类型、候选充分性与本地结构化决策稳定性共同驱动的决策，使端云协同既能在常规控制链路承担受控兜底，又能在低代码生成链路前置产出草案，并始终保持端侧校验和安全门控。

具体而言，交互层负责接收和预处理用户输入。文本输入通过 Web 前端接入，语音输入通过 Vosk 离线 ASR 模型识别，中文和英文模型均在端侧本地运行，不依赖外部网络。ASR 或文本输入进入理解链路前，先经过语言归一化模块，对方言词、口语短句和中英文混合表达进行标准化映射。理解层由 BSR 广召回与 LSR 轻量精排组成：前者负责高召回率、低成本地找出候选动作，后者负责结合环境特征和用户偏好完成低延迟排序。决策层优先进入本地 LLM 结构化决策；只有当本地候选不足、结构化结果置信度不足或即将进入澄清链路时，Router 才触发云端补救或低代码草案生成。执行层通过统一接口把结构化结果映射为设备控制、场景切换与自动化动作；当目标属于门锁、门禁、安防布撤防等安全敏感类别时，执行层之前必须先经过用户确认。记忆层则承担短期状态、偏好持久化、语音纠正写回和 RAG 检索增强，为下一轮决策提供可回注的信息。
...
\begin{enumerate}
    ...
    \item \textbf{轻量精排决定常规控制主路径。} LSR 对候选动作提取环境特征与偏好特征，完成快速排序，并输出置信度。对于设备控制、场景切换和信息查询，系统先由本地规则、候选排序与本地 GGUF 尝试完成结构化决策；只有在本地候选不足、Router 准备进入澄清，或本地结构化决策置信度不足时，才触发云端协同补救。对于低代码场景新建、自动化规则生成和 Change 纠正，云端可先生成结构化草案，随后进入本地校验与用户确认。
...
\textbf{轻量化优先。} 只要低成本模块能够完成任务，就不把问题推给高成本模型。LSR 采用 5 维特征的轻量线性加权排序器，DQN 采用 7-64-64-9 的双层感知机（5257 个可训练参数），向量编码采用 22 MB 的 all-MiniLM-L6-v2，目标是在端侧用“够用、稳定、可部署”的组合替代“更大但更重”的方案。
...
LSR（Lightweight Stage Ranking，轻量精排）模块在 BSR 输出的候选集合基础上进行更精细的排序，其核心是一个上下文感知的 5 维线性加权排序器。
...
在 HomeMind 的整体能力编排中，DQN 不承担主理解链路，也不替代 TAP 规则引擎。它的职责更具体：在“当前环境是否值得主动打扰用户并给出场景建议”这一问题上，提供一个可持续学习的轻量判断模块。TAP 负责确定性的自动化触发，DQN 则负责在有限状态和有限动作空间内学习“何时推荐更合适”，从而为端侧 Agent 补足主动服务能力。

项目选择 DQN 的原因主要有三点。第一，家庭主动推荐问题的状态维度较小，时间、温湿度、在家人数、最近场景和设备活跃度等因素已经能够覆盖大部分判断依据；第二，推荐动作空间稳定，适合用固定输出槽位维护；第三，用户对推荐的接受、拒绝、忽略和纠正都可以自然形成反馈信号，便于长期增量更新。基于这些特点，系统采用 PyTorch 实现的 Double DQN 作为主路径，在缺少 PyTorch 时退化到 NumPy 前向与轻量更新路径，以保证端侧部署连续性。

从工程实现看，DQN 模块使用 7-64-64-9 的双层感知机结构，主 Q 网络共 5257 个可训练参数，按 32-bit 浮点数估算约占 21 KB 内存，目标网络与在线网络同构。这样的规模足以支撑场景级主动推荐，同时不会对端侧内存预算形成明显压力。状态向量和动作空间均采用固定定义，便于回归测试、日志分析和后续策略迭代。

当前状态向量共 7 维，其含义如表所示。

\begin{table}[H]
    \centering
    \caption{状态特征向量 $\mathbf{x} \in \mathbb{R}^{7}$}
    \begin{tabular}{@{}cll@{}}
        \toprule
        \textbf{维度} & \textbf{特征} & \textbf{归一化方式} \\
        \midrule
        $d_0$ & 小时（0--23） & $\text{hour} / 23.0$ \\
        $d_1$ & 温度（°C） & $(\text{temp} - 15) / 20$ \\
        $d_2$ & 在家人数 & $\text{members} / 5$ \\
        $d_3$ & 上一个场景索引 & $(\text{last\_scene} + 1) / 8$ \\
        $d_4$ & 星期几（0--6） & $\text{day} / 6$ \\
        $d_5$ & 湿度（\%） & $\text{humidity} / 100$ \\
        $d_6$ & 设备开合率 & $\frac{\#(\text{设备=开})}{\#(\text{总设备})}$ \\
        \bottomrule
    \end{tabular}
\end{table}

当前实现中，第 7 维特征并不是单个设备状态，而是上下文中所有设备的“开启占比”。这使 DQN 不依赖某一个固定设备，而是用整体家庭运行态近似表示当前活跃程度。状态向量构造见 \texttt{core/dqn/policy.py} 的 \texttt{\_state\_to\_vector} 方法。

在输出侧，DQN 当前维护 9 维动作空间，对应 8 类可推荐场景和 1 个“无推荐”分支。

\begin{table}[H]
    \centering
    \caption{动作映射表 $a \in \{0,\dots,8\}$}
    \begin{tabular}{@{}cll@{}}
        \toprule
        \textbf{动作索引} & \textbf{场景模式} & \textbf{说明} \\
        \midrule
        0 & 睡眠模式 & 卧室调暗、空调睡眠 \\
        1 & 待客模式 & 客厅明亮、音乐播放 \\
        2 & 离家模式 & 关闭非必需设备 \\
...
其中动作 5 被显式保留为“无推荐”分支：若 DQN 认为当前不应打扰用户，系统直接返回空推荐，不进入后续确认或执行流程。需要说明的是，这 9 维输出描述的是 DQN 当前覆盖的“主动推荐动作空间”，并不等同于系统全部场景全集；例如“回家模式”虽然在 SceneSwitcher 与场景配置中受支持，但当前未纳入 DQN 主动推荐动作，而是由用户显式指令、规则链路或其他决策链路触发。动作映射定义见 \texttt{core/dqn/policy.py} 的 \texttt{SCENES} 常量。

训练侧采用 Double DQN + 经验回放。系统用在线网络选择下一动作，用目标网络对下一状态打分，以减轻单网络更新中常见的 Q 值高估问题；损失函数采用 \texttt{smoothL1}，PyTorch 主路径下优化器为 Adam。当前超参数设置为：折扣因子 $\gamma = 0.95$，初始探索率 $\epsilon_0 = 0.3$，最小探索率 $\epsilon_{\min} = 0.05$，学习率 $\alpha = 0.001$，经验回放容量 1000，批量大小 16，每 50 条反馈执行一次在线更新，每 250 条反馈同步一次目标网络。其核心更新关系可概括为：

\[
\begin{aligned}
y &= r + \gamma Q_{\text{target}}\bigl(s', \arg\max_{a'} Q_{\text{online}}(s', a')\bigr) \\
\mathcal{L}(\theta) &= \frac{1}{B} \sum_{i=1}^{B} \operatorname{smoothL1}\!\left(Q_{\text{online}}(s_i, a_i), y_i\right)
\end{aligned}
\]

在运行逻辑上，DQN 每次先根据当前上下文计算各动作的 Q 值，再按式
\[
c = \frac{Q_{\max}}{\sum_{a=0}^{8} |Q(s, a)| + \epsilon}, \qquad \epsilon = 10^{-6}
\]
得到推荐置信度。当前系统采用分级触发策略：当推荐结果为“无推荐”时静默返回；当推荐结果不为“无推荐”且 $c > 0.8$ 时，可直接触发场景切换；当推荐结果不为“无推荐”但 $c \leq 0.8$ 时，则先向用户发起确认。需要说明的是，\texttt{main.py} 中的 \texttt{confidence\_threshold = 0.75} 用于 LLM 结构化决策，不用于 DQN 主动推荐；DQN 主动推荐当前单独使用 0.8 作为自动执行阈值，触发逻辑位于 \texttt{HomeMindAgent.proactive\_recommend}。

用户反馈被直接纳入奖励映射并写入经验回放池，形成持续学习闭环。当前奖励设计如表所示。

\begin{table}[H]
    \centering
    \caption{用户反馈与奖励映射 $r = f(\text{feedback})$}
    \begin{tabular}{@{}cll@{}}
        \toprule
        \textbf{用户反馈} & \textbf{奖励值 $r$} & \textbf{说明} \\
        \midrule
        接受 & $+1.0$ & 用户确认推荐 \\
        忽略 & $0.0$ & 无反馈，中性 \\
        拒绝 & $-0.5$ & 明确拒绝 \\
...
奖励映射由 \texttt{REWARD\_MAP} 与 \texttt{REWARD\_ALIASES} 共同维护，除中文反馈外，还兼容 accepted、ignored、rejected、corrected 等英文别名。整体来看，DQN 在 HomeMind 中并不是追求复杂策略搜索的大模型替代物，而是一个面向家庭场景、强调小规模、可持续更新和可解释触发边界的主动推荐模块。
...
OpenAI 兼容 API 模式（用于云端协同）对接支持 OpenAI API 格式的大模型服务，推理能力更强，需要网络通信和最小必要上下文上传。该模式有两种触发位置：常规控制任务由 Router 在 BSR/LSR 之后、置信度不足以本地直达但仍具备候选基础时触发；低代码场景新建、TAP 自动化规则生成和 Change 纠正等开放式生成任务可在草案生成阶段前置触发，但返回结果只作为待校验草案，不能绕过本地设备、空间、协议、Schema 和安全校验。
...
DeviceController 支持协议网关注入。当传入 \texttt{protocol\_gateway} 参数时，每次状态变更后都会自动同步到真实设备；其中 \texttt{push\_to\_gateway} 负责推送状态变更，\texttt{sync\_from\_gateway} 负责在查询前拉取最新状态。空间设备校验层负责把户型图中的 entity id、设备类型和区域名映射到系统语义设备，例如将 \texttt{light.media\_zone} 映射为“灯光”，将 \texttt{climate.living\_room\_ac} 映射为“空调”。
...
在算法层面，BSR 规则召回的时间复杂度为 O(1)，通过预计算的关键词哈希表实现毫秒级匹配；向量召回阶段使用 all-MiniLM-L6-v2 模型（384 维向量），推理速度较 BERT-base（768 维）提升约 2 倍；LSR 精排采用 5 维线性加权排序器，推理时间约 1 ms；DQN 推理采用“7 输入、64-64 双隐层、9 输出”结构，总计算量仍保持在端侧可接受范围内。
...
为验证系统是否满足赛题 4 GB 内存上限，本项目不采用模型文件大小或参数量估算作为最终依据，而是以运行进程的 RSS 峰值作为判定口径。RSS 能够覆盖 Python 解释器、PyTorch 运行时、embedding 模型权重、ChromaDB 客户端、业务对象、缓存和一次真实查询后的运行态占用，更接近端侧部署时需要预留的实际内存。
...
测试设计分为端到端链路和组件链路两类。端到端链路覆盖 Web Agent、Core Agent 和 Full Local 三种运行形态，用于回答“系统整体运行是否满足 4 GB 上限”；组件链路覆盖 ChromaDB 和 PyTorch，用于观察主要依赖的单项开销。单独 embedding profile 在当前环境中存在长时间等待现象，但端到端 Web Agent 和 Full Local profile 已经实际加载 \texttt{all-MiniLM-L6-v2}，并在日志中出现 BertModel 加载记录，因此 embedding 开销已被纳入端到端峰值，不再作为必要的单项通过条件。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\bfseries}p{3.4cm}X>{\centering\arraybackslash}p{2.7cm}}
        ...
        \midrule
<<<<<<< Updated upstream
        Web Agent & 导入 Web 服务，初始化 HomeMind Agent，加载 embedding 模型，执行 50 条查询 & 1240.40 MB \\
        Core Agent & 导入核心 Agent，完成初始化，执行 50 条核心查询 & 1223.14 MB \\
...
=======
        Web Agent & 导入 Web 服务，初始化 HomeMind Agent，加载 embedding 模型，执行查询 & 982.98 MB \\
        Core Agent & 导入核心 Agent，完成初始化，执行核心查询 & 1211.88 MB \\
        Full Local & 加载 ChromaDB、PyTorch、Web Server 与 HomeMind Agent，不含 llama.cpp 本地大模型 & 808.50 MB \\
        Full Local + llama.cpp & 在 Full Local 运行态基础上加载 1.5B Q4 本地大模型，包含 KV cache 与首轮推理开销 & 2209.48 MB \\
        Embedding & 导入 sentence-transformers，加载 all-MiniLM-L6-v2 并完成一次编码 & 1121.27 MB \\
        PyTorch & 导入 torch，分配矩阵，执行矩阵乘法，初始化 DQNPolicy & 486.33 MB \\
        ChromaDB & 创建持久化 client，写入 100 条测试向量并执行一次查询 & 114.85 MB \\
>>>>>>> Stashed changes
        \bottomrule
    \end{tabularx}
\end{table}
...
<<<<<<< Updated upstream
从测试结果看，当前最高 RSS 峰值出现在 Web Agent 端到端链路，为 1240.40 MB，约占 4096 MB 上限的 30.28\%。该结果已经覆盖 Flask Web 层、HomeMind Agent 初始化、sentence-transformers embedding 模型加载和批量查询后的运行态内存，能够作为当前端侧主链路满足赛题内存约束的主要证据。Core Agent 峰值为 1223.14 MB，与 Web Agent 接近，说明主要内存开销来自模型与 Agent 运行态，而非 Web 服务框架本身。

需要说明的是，本轮测试未包含 llama.cpp 本地大模型权重加载。若最终方案启用本地 GGUF 模型，则必须在安装 \texttt{llama\_cpp} 并指定模型文件后重新运行 llama.cpp profile 和 full local profile，不能将当前 1240.40 MB 结果扩展解释为包含本地 LLM 后端的最终峰值。
=======
从测试结果看，当前最高 RSS 峰值出现在 Full Local + llama.cpp 端到端链路，为 2209.48 MB，约占 4096 MB 上限的 53.94\%。其中 mock 后端 Web Agent 峰值为 982.98 MB，Core Agent 峰值为 1211.88 MB，Full Local 峰值为 808.50 MB，说明在不加载本地 GGUF 大模型时，系统主链运行态整体仍控制在 1--2 GB 常态业务区间以内；而启用 Qwen2.5-1.5B-Instruct-Q4\_K\_M 的 llama.cpp 本地推理后，端侧完整链路仍可稳定落在 4 GB 上限以内。该结果能够作为当前端侧主链与本地 LLM 增强链路满足目标终端内存约束的主要证据。

需要说明的是，本轮测试已经包含 llama.cpp 本地大模型权重加载。当前端侧预设统一为 Qwen2.5-1.5B-Instruct-Q4\_K\_M，采用 2048 上下文和 4 线程；\texttt{Full Local + llama.cpp} profile 给出完整本地链路 2209.48 MB 峰值。由于单独 \texttt{llama.cpp} profile 在最新一次测试中未形成有效结果，本文不单列该项峰值。若后续更换为更大规模 GGUF 模型或调整上下文窗口、线程数和 GPU layers，则需要按同一脚本重新测量新的峰值 RSS、首次推理时延和稳态 p95 指标，不能直接外推当前结果。
>>>>>>> Stashed changes
...
偏好数据（PreferenceStore）以结构化 JSON 快照方式持久化存储，保证设备偏好、场景偏好、推荐接受率和语言纠正映射的统一读写；会话状态由 SessionStore 独立维护，降低短期上下文与长期偏好的耦合；知识库支持 ChromaDB 向量数据库和内存回退两种模式，无 ChromaDB 时自动降级为关键词匹配；加密存储采用 Fernet 对称加密与 PBKDF2-HMAC-SHA256 密钥派生方案，数据加密、完整性保护和存储原子操作共同确保断电安全性。
...
工具的扩展点包括：新增设备类型（在 SVG 户型图设备映射表中新增 entity、type、area 与可选显示名，并在语义设备映射规则中补充类型匹配）、新增场景类型（在 SCENE\_CONFIGS 字典中添加场景配置）、新增信息查询类型（在 InfoQuery.execute 方法中添加分支处理）。扩展过程以户型图设备表为真实设备来源，避免仅修改 DeviceController.state 后出现“代码支持但家庭中不存在”的误执行问题。
...
为兼顾空间理解与运行态控制，系统在设备数据建模上采用双层结构。其一是空间语义设备层，用于维护户型图中的设备映射、区域标识、区域别名与空间归属关系，服务自然语言中的房间级指代解析、设备存在性校验与可视化展示；其二是运行态控制设备层，用于维护控制面板中的设备注册信息、当前状态、增删改查操作与执行入口。前者强调空间语义一致性，后者强调交互与执行稳定性，二者共同支撑家庭场景中的空间感知与设备控制闭环。
...
设备映射支持 Tuple 和 Object 两种导入格式，经归一化处理后按 area 分组，计算每个设备在房间区域内的显示坐标。归一化处理包括 entity\_id 格式校验、area 非空校验、device\_type 支持列表校验以及可选坐标、显示名称、控制属性和区域显示名归一化。区域显示名可通过 \texttt{areaName}、\texttt{roomName} 或顶层 \texttt{areaNames} 字典维护，例如将 \texttt{living\_room} 命名为“影音区”，后续用户说“打开影音区灯”时即可命中该区域。HausRAGen 风格的设备映射数据统一存储在 \path{data/devices.json}，与户型图元数据通过 floor plan id 建立关联。

设备映射数据使系统能够理解"打开客厅灯"等带房间修饰的指令，通过查询 area="living\_room" 或用户自定义区域名对应的设备列表，BSR 与执行前空间校验层可共同定位目标设备。当前实现将激活户型图及其设备映射作为自然语言控制和场景切换的真实设备来源：若当前户型图中没有对应设备，系统不会回退到内置模拟设备，而是返回明确的 unsupported/clarification 响应。设备开关功能模块同时提供设备注册表 CRUD，支持前端对设备的新增、编辑、删除、开关控制和状态刷新；设备 id 和运行态字段（如开关状态、当前状态快照）不允许通过编辑接口修改，运行期设备状态只能由控制接口写入本地注册表文件 \path{data/device-registry.json}。为适配约 100 个设备、10 个空间的演示数据规模，前端设备控制面板采用"先选择空间、再显示该空间设备"的交互方式，按 \texttt{area}/\texttt{areaName} 对设备分组，并使用紧凑设备卡片展示状态、协议、空间和开关操作；编辑表单默认折叠，以减少长列表滚动和同类设备在不同空间下的识别负担。场景配置模块也提供创建、读取、更新和删除能力，场景配置仍以结构化设备动作字典保存，并在切换时经过同一套 SVG 设备表校验。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\bfseries}p{4cm}X>{\centering\arraybackslash}p{2.5cm}}
        ...
        指标 & 说明 & 结果 \\
        \midrule
        自动化测试通过数量 & pytest 全量回归测试覆盖单元、模块、Web API、数据韧性、户型图、设备管理与场景管理流程 & 125 passed \\
...
<<<<<<< Updated upstream
        ...
        端侧推理内存峰值 & Python 3.10 + PyTorch 2.6.0 环境下 Web Agent 加载 embedding 并完成查询后的进程 RSS 峰值；llama.cpp 本地大模型尚未纳入 & 1240.40 MB \\
        偏好学习准确率 & 系统记住并正确应用用户偏好的比例 & $\approx$ 88\% \\
=======
        DQN 参数量 & Q 网络参数数量 & 5257 \\
        端侧推理内存峰值 & Python 3.10 + PyTorch 2.6.0 环境下 mock Web Agent 完成初始化和查询后的进程 RSS 峰值 & 982.98 MB \\
        本地大模型完整链路峰值 & Full Local 运行态叠加 Qwen2.5-1.5B-Instruct-Q4\_K\_M llama.cpp 本地推理的进程 RSS 峰值 & 2209.48 MB \\
...
当前自动化测试共收集并通过 125 个用例，覆盖核心算法模块、Web API、Mock 主流程、TAP 自动化、语音反馈、本地敏感数据保护、知识库恢复、SVG 户型图管理、设备映射、设备注册表增删改查、场景配置增删改查以及空间设备强校验等范围。该结果表明项目已经具备可重复执行的回归测试基础。进一步的 87 条批量指令数据集评测显示，系统总体匹配率由修复前的 85.06\% 提升至 100.00\%，自动化任务创建、聊天回复、澄清请求、英文/口语归一化、场景切换、设备控制、不支持设备识别和上下文跟随八类样本均达到 100\% 通过率。

端侧规则召回在高频明确命令场景下仍然承担主要流量。批量数据集中 60 条请求进入 local 路由，占总样本的 68.97\%；总体 API 平均延迟为 6.211 ms，P95 延迟为 10.643 ms。该结果表明，在家庭场景的高度结构化指令空间中，轻量级规则引擎配合候选排序能够以很低的计算开销完成主要任务闭环。
...
=======
偏好学习机制验证了端侧持续学习的可行性。约 89\% 的偏好学习准确率表明端侧小规模反馈数据足以支撑有效的个性化学习，且无需上传任何隐私数据；同时，Token 压缩结果说明云端补救路径已经被收敛到较小载荷范围，满足“最小必要上传”的设计原则。
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-automation-rule-1.png}
        \caption{自动化规则列表与启停界面}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-automation-rule-2.png}
        \caption{规则编辑与条件配置界面}
    \end{subfigure}
    \caption{HomeMind 自动化规则管理界面截图}
    \label{fig:tap}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui-svg-floorplan.png}
    \caption{SVG 户型图与设备映射配置界面截图}
    \label{fig:svg-floorplan-ui}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-entry-overview.png}
        \caption{HomeMind 入口与主界面}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-dialog-chat.png}
        \caption{自然语言对话界面}
    \end{subfigure}
    \caption{HomeMind 主界面与对话界面截图}
    \label{fig:entry-dialog-ui}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-scene-switch-1.png}
        \caption{场景模式列表与选择界面}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-scene-switch-2.png}
        \caption{场景切换后的联动展示界面}
    \end{subfigure}
    \caption{场景切换界面截图}
    \label{fig:scene-switch-ui}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-device-panel-1.png}
        \caption{空间入口视图}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-device-panel-2.png}
        \caption{空间内设备卡片视图}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{ui-device-panel-3.png}
        \caption{设备编辑与状态操作视图}
    \end{subfigure}
    \caption{设备控制面板截图}
    \label{fig:device-panel-ui}
\end{figure}

场景八（记忆与偏好回顾）：用户可以在记忆与偏好界面查看近期会话摘要、长期偏好和已写回的家庭经验条目。该界面用于展示系统如何将用户纠正、场景接受记录和设备使用习惯沉淀为后续可复用的本地记忆，从而支撑下一轮召回、排序和决策增强。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\linewidth]{ui-memory-preference.png}
    \caption{记忆摘要与偏好持久化界面截图}
    \label{fig:memory-preference-ui}
\end{figure}

场景九（隐私与云调用状态）：用户可以在隐私与云调用状态界面查看当前请求是否出端、云端协同是否开启、最小必要上传策略是否生效，以及相关隐私保护与调用状态摘要。该界面用于直观展示系统在“本地优先、必要时上云”策略下对隐私边界和云调用行为的受控管理。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\linewidth]{ui-privacy-cloud-status.png}
    \caption{隐私与云调用状态界面截图}
    \label{fig:privacy-cloud-status-ui}
\end{figure}
...
\begin{itemize}
    ...
    \item \verb|/api/floor-plans/\{id\}/activate|：激活指定户型图，后续自然语言设备控制和场景切换以该户型图的设备映射表作为真实设备来源。
